...
This skill converts a T-Bank operations export into iaam operations. It knows
only the institution's export format. Account names and identifiers are runtime
inputs, not skill data.
...
Create an account map outside this skill. Its keys are names from the export and
...
```bash
IAAM_TOKEN=... python3 .claude/skills/tbank-csv-import/import.py \
  --export /path/to/export.csv \
  --account-map /path/to/account-map.json \
...
python3 .claude/skills/tbank-csv-import/import.py \
  ...
  --account-map .claude/skills/tbank-csv-import/fixtures/account-map.json \
  --counterparty-map .claude/skills/tbank-csv-import/fixtures/counterparty-map.json \
  --dry-run
```
...
and is never written into this skill.
