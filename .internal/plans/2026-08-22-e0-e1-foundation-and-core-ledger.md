...
**Acceptance Criteria:**
- **Каждое событие, на котором доказывается классификация, проходит `validate_structure`.** `classify` ничего не валидирует, поэтому тесты на невалидных событиях зелёные — и доказывали бы классификацию на данных, которые журнал не принял бы. `sample_event` даёт одну ногу от `CashIn`; для перевода нужны две встречные ноги на объявленных счетах
- **Мутационный заслон здесь почти слеп** (см. §15.7 спеки): единственный мутант `classify` нежизнеспособен. Гарантию даёт табличный тест на все шестнадцать сочетаний, а не «выживших нет»
...
- **Округление зафиксировано тестом на ничью.** Тесты плана дают только точные деления (`90000·5/10`), поэтому подмена стратегии округления или потеря масштаба ими не ловится. Нужны случаи `2,5 → 2` и `7,5 → 8`: это ровно та функция, что сверяется с независимым эталоном в задаче 14
...
- **Фикстура ловит все четыре подмены стратегии округления.** Проверено: пяти случаев недостаточно — ничья вверх (`15/2 → 8`) отличает `MidpointNearestEven` от `TowardZero` и `ToZero`, а неделимый остаток — от `AwayFromZero`. Без шестого случая parity-тест ловил только одну подмену из четырёх, и слой §15.4 своей работы не делал
- **У эталона есть собственные тесты, не сводящиеся к сверке с продакшеном.** Эталон, покрытый только parity, сам не проверен: ветка `InsufficientQuantity` не вызывалась ни разу, потому что в фикстуре нет перепродажи. `iaam-oracle` входит в список критичных модулей `check-mutants.sh` наравне с продакшеном — ошибка в эталоне маскирует ошибку в продакшене ровно так же, как наоборот
...
```json
{
  ...
  "cases": [
    ...
    {
      "name": "неделимый остаток, дробь меньше половины",
    ...
    {
      "name": "ничья округляется к чётному вверх",
      "_manual_calculation": "лот 2 шт за 15 копеек; продаём 1 → 15*1/2 = 7,5 → ничья, соседи 7 и 8, чётный 8; остаётся 7",
      "lots": [{ "quantity": 2, "basis_minor": 15 }],
      "sell_quantity": 1,
      "expected_basis_released_minor": 8,
      "expected_remaining": [{ "quantity": 1, "basis_minor": 7 }]
    },
    {
...
Временно замените в `split_basis` возврат `Ok(Money::new(PostedMinor::new(value), total.currency()))` на `Ok(Money::new(PostedMinor::new(value + 1), total.currency()))`.

> `PostedMinor(value + 1)` **не компилируется** — поле приватно с задачи 8
> (`E0423`). Только через `PostedMinor::new(...)`.

```bash
nix develop -c cargo nextest run --package iaam-core --test properties
```

Ожидается: падает **только детерминированный тест**. Свойства остаются зелёными, и это не дефект прогона.

> **Свойство сохранения структурно неспособно поймать этот слом.** Невыбывшая
> часть считается как `cost_basis.try_sub(taken_basis)` — от того же значения,
> которое вернуло `split_basis`, поэтому их сумма равна исходной при любом
> его результате. Проверено: 200 000 случаев со сломанным разнесением —
> свойство зелёное.
>
> Свойство при этом не бесполезно: оно проверяет, что ни один лот не потерян
> и не учтён дважды при переходе `lots → disposed + remaining`. Но его область —
> целостность перехода, а не величина разнесения. Величину ловит
> детерминированный тест с посчитанным вручную ожиданием, и только он.
