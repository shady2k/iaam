...
> **Редакция.** Указана `2021` как заведомо поддерживаемая. Если закреплённый тулчейн поддерживает `2024` — смените и убедитесь, что `cargo build` зелёный. Не меняйте вслепую.
...
- Тулчейн: <вставьте вывод `nix develop -c rustc --version`>
- Редакция: <2021 или 2024, по факту>
...
Подставьте фактические значения, а не заглушки.
...
```bash
cd .. && cd iaam && cargo --version
```

Ожидается: версия печатается **без** обёртки `nix develop -c`.
...
if ! git rev-parse --verify "$BASE" >/dev/null 2>&1; then
  echo "ОШИБКА: база $BASE недоступна. Заслон не может отработать." >&2
  exit 1
fi
...
added=$(git diff "$BASE"...HEAD -- '*.rs' | grep '^+' | grep -v '^+++' || true)
...
policy_files=$(git diff --name-only "$BASE"...HEAD -- \
  '.github/workflows/**' 'scripts/**' 'deny.toml' 'clippy.toml' \
  '.cargo/mutants.toml' 'Cargo.toml' 'tests/fixtures/**' || true)
...
> **Про `_ =>` в доменных enum.** Этот заслон не ловится текстовым поиском без ложных срабатываний (в `match` по строкам и внешним типам `_ =>` законен). Он закрывается иначе: доменные `enum` в `iaam-core` помечаются `#[non_exhaustive]`-**нет**, а наоборот — остаются исчерпаемыми, и `clippy::wildcard_enum_match_arm` включается точечно в модулях, где это важно. Реализуется в Task 10 вместе с семейством событий.
...
MANIFEST="tests/fixtures/MANIFEST.sha256"
...
if ! sha256sum -c "$MANIFEST" --quiet; then
  ...
  echo "  sha256sum tests/fixtures/*.json > $MANIFEST" >&2
  exit 1
fi
...
> **Чего этот заслон не ловит и как это закрывается.** Агент может изменить
> фикстуру и **тем же коммитом** пересчитать манифест — `sha256sum -c`
> станет зелёным. Слова «отдельным коммитом» механически не проверяются.
> Поэтому изменение содержимого `tests/fixtures/**` или манифеста ловится
> заслоном политики в `check-diff-lint.sh` и требует переменной
> `POLICY_CHANGE_APPROVED=1`, которую выставляет владелец, а не агент.
>
> Проверка «фикстура упоминается в тестах» тоже слабая: упоминание в
> комментарии её удовлетворит. Полноценное решение — тест, сообщающий
> список фактически загруженных фикстур, — относится к задаче 14,
> где появляется первая настоящая фикстура.
...
# ловится заслоном политики в check-diff-lint.sh.
MODULES=(
...
for module in "${MODULES[@]}"; do
  [ -f "$module" ] || { echo "пропуск (ещё не создан): $module"; continue; }
  echo "=== $module ==="
  out_dir="target/mutants/$(echo "$module" | tr '/' '_')"
  if cargo mutants --package iaam-core --file "$module" \
       --output "$out_dir" --no-shuffle; then
    echo "  выживших нет"
  else
    echo "  ВЫЖИВШИЕ МУТАНТЫ в $module" >&2
    if [ -f "$out_dir/outcomes.json" ]; then
      jq -r '.outcomes[] | select(.summary=="MissedMutant") | .scenario.Mutant.function.function_name' \
        "$out_dir/outcomes.json" 2>/dev/null | sort -u >&2 || true
    fi
    fail=1
  fi
done
...
echo "Мутационное тестирование пройдено по всем модулям."
```

> Ключ `--file` и формат `outcomes.json` версионно-зависимы. Проверьте
> `cargo mutants --help` для закреплённой версии и убедитесь через
> `cargo mutants --list --file <модуль>`, что в список не попадают мутанты
> из соседних файлов.
...
- `Dec::to_exact` не теряет точность для значений с масштабом ≤ 28
...
Ожидается: ошибка компиляции `cannot find type Exact`.
...
- `PostedMinor` нельзя сконструировать из `Dec` без явного округления с указанием режима
...
mod tests {
    ...
    fn ids_of_different_kinds_are_distinct_types() {
        // Проверка компиляцией: следующая строка не должна компилироваться,
        // и это зафиксировано в trybuild-тесте задачи 19.
