...
Часть A — ядро
  1 остатки и позиции ─┐
  2 книга лотов ───────┤
  3 потоки контура ────┼─→ 5 снимок, project/advance, инварианты ─┐
  4 оценка и Valuation ┘                                          │
...
Задачи 1–4 независимы друг от друга и могут идти параллельно. Задача 6 не зависит ни от чего в части A. Задача 5 — самая крупная: три файла и две группы тестов; разделять её нельзя, потому что снимок без проверки инвариантов является ровно тем, что §15.2 запрещает выдавать наружу.
...
Пустые заглушки `src/reference.rs`, `src/snapshots.rs`, `src/tokens.rs` создаются в этой задаче с одной строкой `//! Заполняется задачей 11.` — модули объявлены в `lib.rs`, и без файлов крейта не собирается.
