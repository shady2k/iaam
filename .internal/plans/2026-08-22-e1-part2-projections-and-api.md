...
- **измерено при исполнении (2026-08-23): 35 модулей, 1051 мутант, ноль выживших, около двадцати пяти минут.** Решение владельца «полный прогон на каждом PR» в таймаут шестьдесят минут укладывается. По дороге найдены и устранены два ускорителя цены, каждый из которых сам по себе делал прогон невозможным: прогон тестов всего воркспейса на каждого мутанта (`--test-workspace true`) поднимал цену в девять раз — заменён поимённым указанием пакетов там, где тесты модуля лежат в другой крейте; заслон `trybuild` компилировал четыре случая по одиннадцать секунд на каждого из тысячи мутантов — исключён из мутационного прогона (но не из обычного) с обоснованием в `.cargo/mutants.toml`. Охват не уменьшен ни разу;
...
> **Приёмка пересмотрена позже (2026-08-26, бид `iaam-46a`).** Второй критерий — «полными считаются только исполнимая цена и цена закрытия» — больше не действует в этой формулировке: полнота и исполнимость разведены на две независимые оси. Текст выше оставлен как запись того, что было принято при исполнении задачи. Действующая редакция — §5.3 и §5.4 спецификации; разбор — `docs/decisions/0002-polnota-ocenki-i-ispolnimost-ceny-dve-osi.md`. Код (`PriceQuality::is_complete`) меняется в E3.3, а не этим пересмотром.
...
> **Исправление при исполнении (2026-08-23).** `lib.rs` этой задачи уже зовёт `auth::authenticate` и `rate_limit::RateLimiter`, которые заводит задача 16, — «зелёная сборка» шага 7 без неё недостижима. Задачи 15 и 16 выполняются и коммитятся вместе.
...
> **Исправление при исполнении (2026-08-23).** Обнаружен дефект самого мутационного заслона: `check-mutants.sh` звал `cargo mutants --package X --file Y`, а этот флаг ограничивает **и прогон тестов** тем же пакетом. Для ядра это совпадало случайно — его тесты лежат в нём же, — но модули оболочки проверяются контрактными тестами из `iaam-server`, и заслон печатал бы «выживших нет» для кода, который никто не тестировал. Проверено исполнением: 46 выживших против 35 на одном и том же коде. Добавлен `--test-workspace true`.
>
> Крейты `iaam-app` и `iaam-server` в заслон не входили вовсе. Восемь их файлов внесены в `MODULES`. Выжившие закрыты: области действия токена и коды ошибок табличными тестами; ограничитель частоты — проверкой публичной точки входа и обеих границ окна; вердикты и пояснения отказов — тестами DTO; права администратора, нумерация строк, справочник CSV, разбор `as_of`, список счетов, снимок и журнал обращений — семью контрактными тестами, из них один на базе файлом со вторым соединением.
>
> Два мутанта объявлены эквивалентными с письменным обоснованием в описании бида iaam-1fk.18 и в комментарии заслона: чтение снимка и ранний возврат при нарушении инварианта меняют объём работы, а не ответ. Их файлы в `MODULES` не внесены, и это записано рядом со списком, а не умолчано.
>
> Из порта `Store` удалён метод `load_events`: его не звал никто. Мёртвый метод порта нечем проверить и незачем держать.

**Почему `iaam-bootstrap` — отдельная крейта.**
...
> **Исправление при исполнении (2026-08-23).** План отмечал содержимое этой задачи как «написано, но не сверялось с поднятым сервисом». Сверка проведена и нашла дефект контракта: отказ приёмки называл внутреннее имя поля (`amount_minor`) и печатал величину в минимальных единицах (`-500`), хотя клиент присылал `amount` и `"-5.00"`. Агент показал бы владельцу поле, которого тот не отправлял. Исправлено: отказ говорит теми же именами и единицами, что и запрос. В сам скилл добавлен раздел «Что здесь проверено».
...
> **Исправление при исполнении (2026-08-23).** Названного здесь теста план ни в одной задаче не создавал: инвариант существовал как код, но проверки его срабатывания не было. Тест написан. Расхождение приходится строить вручную: структурная проверка после задачи 4 не пропускает противоречивое событие в журнал вовсе, и инвариант стал вторым рубежом — но второй рубеж, который никто не проверял, рубежом не является.
>
> При написании теста найдена дыра в самом инварианте: обход идёт по записям книги лотов, поэтому позиция, у которой записи в книге нет вовсе, не сравнивается ни с чем. Через журнал сегодня недостижимо, заведён бид.
