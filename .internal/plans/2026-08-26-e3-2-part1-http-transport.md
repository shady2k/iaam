...
**Порядок и параллельность.** Задача 1 первая. Задачи 2 и 4 стоят на ней и между собой независимы — идут параллельно. Задача 3 стоит на задаче 2 (ей нужен `trust::client_for`). Задача 7 последняя: заслон, добавленный раньше, покраснеет на ещё не переселённом брокере.

**Задачи 5 и 6 объединены в одну и выполняются одним воркером.** Причина обнаружена при исполнении и является дефектом первоначального разреза: задача 2 уносит сертификат из `crates/iaam-broker/certs/`, а `crates/iaam-broker/src/trust.rs` ссылается на него через `include_str!`. С момента слияния задачи 2 крейта брокера **не собирается** и остаётся такой до удаления этого файла — то есть до задачи 6. Поэтому задача 5 не может выполнить собственную приёмку «все существующие тесты брокера проходят»: крейта красная по причине, к задаче 5 отношения не имеющей. Независимо проверяемого результата у задачи 5 в одиночку нет, а это условие правильного разреза задач.
...
### Task 5+6: Переселение брокера на `iaam-http` (одна задача)

> Задачи 5 и 6 исходного плана объединены: см. «Порядок и параллельность» выше.
> Брокер не собирается между ними, поэтому промежуточного проверяемого
> состояния не существует. Воркер выполняет оба блока шагов подряд
> и проверяет крейту один раз, в конце.

#### Часть А — клиент Т-Инвестиций
...
#### Часть Б — клиент Finam и снятие `reqwest`
