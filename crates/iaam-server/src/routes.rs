...
#[utoipa::path(
    ...
    responses(
        ...
        (
            ...
            body = ApiError,
            headers(("Retry-After" = u64, description = "Whole seconds to wait before calling again; only with `source_unavailable`, and only when the wait is known"))
        ),
        (status = 400, description = "Request body could not be read", body = ApiError),
        (status = 413, description = "Request body exceeds the limit", body = ApiError),
        (status = 415, description = "Body sent without Content-Type: application/json", body = ApiError),
...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 502, description = "The market source refused the request (code `source_refused`): a retry gets the same answer", body = ApiError),
        (
            status = 503,
            description = "Either the market transport is not configured (code `not_configured`), or the market source stayed unreachable through every retry (code `source_unavailable`); no observation was recorded: call again after the seconds in Retry-After, which is sent when the wait is known",
            body = ApiError,
            headers(("Retry-After" = u64, description = "Whole seconds to wait before calling again; only with `source_unavailable`, and only when the wait is known"))
        ),
        (status = 400, description = "Request body could not be read", body = ApiError),
