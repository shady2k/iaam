...
impl SqliteStore {
    ...
    ///
    /// Три роли валюты заполняются одним значением, а род пишется `NULL`:
    /// `InstrumentRecord` их пока не различает. Это временное состояние
    /// на время волны 1 E3.1 — различение приходит вместе с
    /// `CurrencyRoles` в следующей задаче. Не принимайте это за решение
    /// «у инструмента всегда одна валюта»: колонки разведены именно
    /// потому, что у замещающих облигаций они расходятся (§7.2).
