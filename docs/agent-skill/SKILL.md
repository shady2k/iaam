...
У операции `opening_position` есть необязательное поле `assertions` —
то, что владелец утверждает о позиции, существовавшей до начала журнала.
Отсутствующее поле означает, что владелец не утверждал ничего. Это
законное состояние, а не пробел в запросе: не заполняйте его догадками.

| Поле | Значения | О чём оно |
|---|---|---|
| `quantity` | `known`, `estimated` | количество точное или оценённое |
| `acquisition_date` | дата `YYYY-MM-DD` | когда бумага приобретена |
| `acquisition_date_certainty` | `known`, `estimated`, `unknown` | насколько владелец уверен в этой дате |
| `tax_basis` | `documented`, `estimated`, `unknown` | подтверждена ли стоимость приобретения документом |
| `basis_currency`, `basis_rate` | код валюты, строка-число | в какой валюте и по какому курсу известна стоимость |
| `fees_included` | `yes`, `no`, `unknown` | входят ли комиссии в стоимость |
| `ldv_eligibility` | `known`, `unknown` | известно ли право на льготу долгосрочного владения |
| `prior_corporate_actions` | `known`, `unknown` | известны ли корпоративные действия до начала журнала |

По умолчанию каждое поле стоит в самом незнающем значении (`estimated`
для количества, `unknown` для остальных). Умолчание сохраняет незнание,
а не выводит уверенность из заполненности соседних полей.

Заявленное здесь доходит до сверки выплат. Без даты приобретения
границу владения провести нечем, и выплаты по такой бумаге попадут в
...
`unknown`, а не подставляйте начало журнала.
...
интервала, источник, `observed_at`, качество и `complete_through`. Для цен
`quotation_basis` — эффективное основание, `recorded_quotation_basis` —
основание ровно в записи источника, а `quotation_basis_status` — машинный
статус доказанности: `proven`, `contradicts` или `not_proven`. У ключевой
