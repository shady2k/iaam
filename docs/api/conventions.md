...
| `GET /v1/contours/{contour}` | `ContourDto` | object, `accounts` | the contour's identity, `title` and `version` beside the composition, and `same_title_contours` — the other contours of this owner sharing its title, which no account in the composition can say |
