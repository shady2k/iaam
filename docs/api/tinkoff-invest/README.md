...
В песочнице вызываются **обычные** методы по песочному адресу — это
рекомендуемый Т-Инвестициями способ, а не методы `SandboxService`.
Два метода живут только на боевом контуре: `GetBrokerReport`
и `GetDividendsForeignIssuer`. Токен у контуров **свой**: песочный
на боевых методах и боевой на песочных дают отказ.

Т-Инвестиции зовут это контуром; внутри системы то же самое называется
**средой** (`iaam_broker::environment::Environment`), потому что слово
«контур» здесь занято составом контура учёта (§11).

Отказы шлюза различать по коду ненадёжно: на одном и том же токене
он отвечает то `401` `40003`, то `500` `70001`. Проверять годность
токена нужно повтором, а не толкованием кода.
