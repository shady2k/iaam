...
```bash
export IAAM_DATABASE=/var/lib/iaam/iaam.db
export IAAM_BROKER_KEY_OLD_FILE=/etc/iaam/broker-key
export IAAM_BROKER_KEY_NEW_FILE=/etc/iaam/broker-key.next
iaam
```
...
3. при необходимости заранее создайте новый ключ и выполните штатную
   ротацию через `IAAM_BROKER_KEY_OLD_FILE` и
...
Проверить заведённый доступ на живом шлюзе: `make sandbox`. Это проверяет
ключ, шифротекст, корень доверия и авторизацию, но не импортирует
операции. Клиенты и сценарий API-канала существуют отдельно от
отчётного парсера, однако публичный `POST /v1/brokers/{broker}/sync` пока
отвечает `501`, а фонового планировщика нет.
