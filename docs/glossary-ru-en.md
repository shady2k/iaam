...
| отзыв счёта | account retraction | `iaam_core::retraction`; the owner's, or the declaring agent's, statement that an account row should never have existed. **A third axis, not a spelling of retirement or of a scope exclusion**: retirement says the product existed and ended, a scope exclusion says it exists and the owner's reports leave it out, this says there was never an account here at all — it removes the account from a report's population outright rather than annotating it there (`iaam-o0oj`). Deliberately the same word the journal's own **retraction** (`HistoryAct::Retracted`, below) uses, and for the same reason: both are the declarer withdrawing something of its own as a set, never an owner's judgement about a fact he did not write |
...
| история операции | operation history | `OperationHistory`; one operation's life read forwards — what it was when it arrived, every act since, and the fact that counts now. Entered by any identifier in the chain, because the original, the replacement standing now and every reversal all name the same operation. Never a listing of facts: the facts are what the acts wrote |
| действие (над операцией) | act | `HistoryAct`; one thing the **owner** did to an operation, as against one fact the journal holds. A correction is normally two facts and is one act, and the folding is done once so that no reader downstream has to work out that the two belong together |
| появление | arrival | `HistoryAct::Arrived`; the act that put the fact in the journal. The first step of every history, and also the word for a history beginning at a fact whose target this journal does not hold |
| исправление | correction | `HistoryAct::Corrected`; a replacement of the target, and the reversal beside it where one was written. The replacement is what makes the act; a reversal alone is a **retraction** |
| отзыв | retraction | `HistoryAct::Retracted`; a reversal with no replacement — the fact stopped counting and nothing took its place, so there is no state after it. A whole import taken back writes the same fact one corrected row does, and this is the word for both |
| изменённая грань | changed aspect | `ChangedAspect`; which aspect of the fact an act made different — kind, amount, account, dates, source description, confidence. A closed vocabulary that **names** where the difference is and never renders it: both states are published beside it, and a second rendering would come to disagree with them. A category is deliberately not among them, because a category is not recorded on the fact at all |
| момент записи | recorded moment | `recorded_at`; when this instance wrote a fact down. Never an **effective date** and never published as one: a correction written in March to a fact effective in January is a March act about a January fact, and confusing the two dates the owner's change of mind to the day of the operation he changed his mind about |
...
| основание факта | fact basis | `FactBasis`; on whose word a row was settled — the caller concluded, the directory recognised, the source asserted, a rule matched, the owner answered, or the owner answered and that same answer minted a rule. Six, and the last two are kept apart because they are the same claim about **who decided** and different claims about what group the row belongs to. Published beside the event kind, which says what the fact is and never why it may be written (decision 0031 §7) |
| правило, порождённое ответом | rule an answer minted | `AnswerRule`, `FactBasis::AnsweredMintingRule`, `RuleSettlement::AnsweredMintingRule`; the standing rule the owner's own answer wrote, recorded as a pair — the identifier and the version it was minted at — on every row that answer settled. The pair and never half of it: an identifier with no version, or a version naming no rule, is a state with no truthful answer. It names one decision of his, and the group that decision reached is the rows the answer settled plus the rows the rule went on to file |
...
| прогноз стоящего решения | standing-decision forecast | `AnswerRuleForecast`; what the standing decision one answer would keep would settle, asked **before** the answer is given — the lines of this import it would cover, the movements already recorded it would cover, and what could not be judged either way. Read-only: it keeps no decision, answers nothing and corrects nothing, and correcting a movement stays an act of its own beside changing the decision behind it. The counterpart of a **category rule preview** for the other rule vocabulary |
| не поддаётся суждению | undecided | `Undecided`; something a forecast could not test the condition against — a line this build cannot read, a movement recorded before the source's two words were kept in separate fields, or a record that would not fold at all. Published with its reason and never omitted: an omission reads as «nothing else is affected», which is the one thing a forecast must not say |
